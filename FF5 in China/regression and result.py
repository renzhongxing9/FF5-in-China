# @ Time: 2024/4/30 17:43
# @ File: regression and result.py
# @ Author: ZHONGXING REN
# @ Insitution: Dongbei university of Finance & Economics
import pandas as pd
import numpy as np
import os
...
for fac in ['BM', 'Inv', 'OP']:
    ...
    if fac == 'BM':
        Size_BM = data1._append(data2)
    elif fac == 'Inv':
        Size_Inv = data1._append(data2)
    elif fac == 'OP':
        Size_OP = data1._append(data2)
# 对BM_Size分组进行五因子回归
...
for i in range(1, x_ + 1):
    for j in range(1, y_ + 1):
        ...
        model = sm.OLS(y, X)
        results = model.fit()
        regression_BM = regression_BM._append([{'Size': i, 'BM': j, 'Const': results.params[0],
        ...
        regression_BM_pvalue = regression_BM_pvalue._append([{'Size': i, 'BM': j, 'Const_p': results.pvalues[0],
                                                             'Mktrf_p': results.pvalues[1], 'SMB_p': results.pvalues[2],
                                                             'HML_p': results.pvalues[3], 'RMW_p': results.pvalues[4],
                                                             'CMA_p': results.pvalues[5]}])
        regression_BM_tvalue = regression_BM_tvalue._append([{'Size': i, 'OP': j, 'Const_t': results.tvalues[0],
...
for i in range(1, x_ + 1):
    for j in range(1, y_ + 1):
        ...
        model = sm.OLS(y, X)
        results = model.fit()
        regression_Inv = regression_Inv._append([{'Size': i, 'Inv': j, 'Const': results.params[0],
                                                 'Mktrf': results.params[1], 'SMB': results.params[2],
                                                 'HML': results.params[3], 'RMW': results.params[4],
                                                 'CMA': results.params[5], 'R-squared': results.rsquared}])
        regression_Inv_pvalue = regression_Inv_pvalue._append([{'Size': i, 'Inv': j, 'Const_p': results.pvalues[0],
                                                               'Mktrf_p': results.pvalues[1],
                                                               'SMB_p': results.pvalues[2], 'HML_p': results.pvalues[3],
                                                               'RMW_p': results.pvalues[4],
                                                               'CMA_p': results.pvalues[5]}])
        regression_Inv_tvalue = regression_Inv_tvalue._append([{'Size': i, 'OP': j, 'Const_t': results.tvalues[0],
                                                                     'Mktrf_t': results.tvalues[1], 'SMB_t': results.tvalues[2],
                                                                     'HML_t': results.tvalues[3], 'RMW_t': results.tvalues[4],
                                                                     'CMA_t': results.tvalues[5]}])
# 对OP_Size分组进行五因子回归
...
for i in range(1, x_ + 1):
    for j in range(1, y_ + 1):
        ...
        model = sm.OLS(y, X)
        results = model.fit()
        regression_OP = regression_OP._append([{'Size': i, 'OP': j, 'Const': results.params[0],
                                               'Mktrf': results.params[1], 'SMB': results.params[2],
                                               'HML': results.params[3], 'RMW': results.params[4],
                                               'CMA': results.params[5], 'R-squared': results.rsquared}])
        regression_OP_pvalue = regression_OP_pvalue._append([{'Size': i, 'OP': j, 'Const_p': results.pvalues[0],
                                                             'Mktrf_p': results.pvalues[1], 'SMB_p': results.pvalues[2],
                                                             'HML_p': results.pvalues[3], 'RMW_p': results.pvalues[4],
                                                             'CMA_p': results.pvalues[5]}])
        regression_OP_tvalue = regression_OP_tvalue._append([{'Size': i, 'OP': j, 'Const_t': results.tvalues[0],
                                                                 'Mktrf_t': results.tvalues[1], 'SMB_t': results.tvalues[2],
                                                                 'HML_t': results.tvalues[3], 'RMW_t': results.tvalues[4],
                                                                 'CMA_t': results.tvalues[5]}])
...
model = sm.OLS(y,X)
results = model.fit()
regression_factors = regression_factors._append([{'DepVar': 'Mktrf','Const': results.params[0],
                                                 'Mktrf': 0, 'SMB': results.params[1],
                                                 'HML': results.params[2], 'RMW': results.params[3],
                                                 'CMA': results.params[4], 'R-squared': results.rsquared}])
regression_factors_tvalue = regression_factors_tvalue._append([{'DepVar': 'Mktrf','Const_t': results.tvalues[0],
                                                               'Mktrf_t': 0, 'SMB_t': results.tvalues[1],
                                                               'HML_t': results.tvalues[2], 'RMW_t': results.tvalues[3],
                                                               'CMA_t': results.tvalues[4]}])
regression_factors_pvalue = regression_factors_pvalue._append([{'DepVar': 'Mktrf','Const_p': results.pvalues[0],
...
model = sm.OLS(y,X)
results = model.fit()
regression_factors = regression_factors._append([{'DepVar': 'SMB','Const': results.params[0],
                                                 'Mktrf': results.params[1], 'SMB': 0,
                                                 'HML': results.params[2], 'RMW': results.params[3],
                                                 'CMA': results.params[4], 'R-squared': results.rsquared}])
regression_factors_tvalue = regression_factors_tvalue._append([{'DepVar': 'SMB','Const_t': results.tvalues[0],
                                                               'Mktrf_t': results.tvalues[1], 'SMB_t': 0,
                                                               'HML_t': results.tvalues[2], 'RMW_t': results.tvalues[3],
                                                               'CMA_t': results.tvalues[4]}])
regression_factors_pvalue = regression_factors_pvalue._append([{'DepVar': 'SMB','Const_p': results.pvalues[0],
...
model = sm.OLS(y,X)
results = model.fit()
regression_factors = regression_factors._append([{'DepVar': 'HML','Const': results.params[0],
                                                 'Mktrf': results.params[1], 'SMB': results.params[2],
                                                 'HML': 0, 'RMW': results.params[3],
                                                 'CMA': results.params[4], 'R-squared': results.rsquared}])
regression_factors_tvalue = regression_factors_tvalue._append([{'DepVar': 'HML','Const_t': results.tvalues[0],
                                                               'Mktrf_t': results.tvalues[1], 'SMB_t': results.tvalues[2],
                                                               'HML_t': 0, 'RMW_t': results.tvalues[3],
                                                               'CMA_t': results.tvalues[4]}])
regression_factors_pvalue = regression_factors_pvalue._append([{'DepVar': 'HML','Const_p': results.pvalues[0],
                                                               'Mktrf_p': results.pvalues[1], 'SMB_p': results.pvalues[2],
...
model = sm.OLS(y,X)
results = model.fit()
regression_factors = regression_factors._append([{'DepVar': 'RMW','Const': results.params[0],
                                                 'Mktrf': results.params[1], 'SMB': results.params[2],
                                                 'HML': results.params[3], 'RMW': 0,
                                                 'CMA': results.params[4], 'R-squared': results.rsquared}])
regression_factors_tvalue = regression_factors_tvalue._append([{'DepVar': 'RMW','Const_t': results.tvalues[0],
                                                               'Mktrf_t': results.tvalues[1], 'SMB_t': results.tvalues[2],
                                                               'HML_t': results.tvalues[3], 'RMW_t': 0,
                                                               'CMA_t': results.tvalues[4]}])
regression_factors_pvalue = regression_factors_pvalue._append([{'DepVar': 'RMW','Const_p': results.pvalues[0],
                                                               'Mktrf_p': results.pvalues[1], 'SMB_p': results.pvalues[2],
...
model = sm.OLS(y,X)
results = model.fit()
regression_factors = regression_factors._append([{'DepVar': 'CMA','Const': results.params[0],
